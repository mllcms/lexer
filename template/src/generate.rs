...
use crate::{replace, replace::Replace};
...
impl Generate {
    pub fn run(self) -> anyhow::Result<()> {
        ...
        let mod_name = format!("mod {};", self.name);
        if !mod_data.contains(&mod_name) {
            fs::write(mod_path, format!("{mod_name}\n{mod_data}"))?;
        }
...
fn generate(input: PathBuf, output: PathBuf, reps: &Replace) -> anyhow::Result<()> {
    fs::read_dir(&input)?.par_bridge().flatten().for_each(|entry| {
